...
namespace StorageWars
{
    public class RoundManager 
    {
        ...
        public float GetInflationMultiplier() => 1.0f + (CurrentRound * GameConstants.InflationRate); // Geçen turlara endeksli bir çarpan döner
        public int CalculateCurrentItemValue(Item item) => (int)(item.Value * GetInflationMultiplier()); // Enflasyon çarpanını eşyanın taban fiyatına uygular

        public int GetBaseStartingPrice() // Her tur müzayedede açılış fiyatını gitgide yukarıya çeker
        {
            return GameConstants.AuctionBaseStartingPrice + ((CurrentRound - 1) * GameConstants.AuctionPriceRoundMultiplier);
        }

        public int GetPlayerBidIncrement() // Oyuncuların Shift'e bastığında artıracakları teklifin limitlerini enflasyona oranla esnetir
        {
        ...
        public int GetBotBidIncrement() // Botun teklif miktar limitlerini esnetir
        {
