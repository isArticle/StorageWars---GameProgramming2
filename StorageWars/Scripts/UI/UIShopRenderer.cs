...
namespace StorageWars
{
    ...
    {
        public void Draw(SpriteBatch spriteBatch, Player p1, Player p2, ShopManager shop) // Dükkan menüsünü ve P1/P2 bütçelerini çizer
        {
        ...
        private void DrawShopSlots(SpriteBatch sb, IReadOnlyList<Skill> skills, Vector2[] slotCoords, Vector2[] nameCoords, Vector2[] priceCoords, Vector2[] cursorCoords, int selectedIndex, Color cursorColor, int playerIndex) // Satılık 3 yeteneği ve dönen seçim imlecini renderlar
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                if (selectedIndex == i)
                {
                    sb.DrawString(AssetManager.GameFont, ">", cursorCoords[i], cursorColor, rotation, cursorOrigin, 1.0f, SpriteEffects.None, 0f); // Seçimi gösteren üçgeni döndürerek çizer
                }
            }
        }
    }
