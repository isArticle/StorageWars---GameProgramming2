...
namespace StorageWars
{
    ...
    {
        public void Draw(SpriteBatch spriteBatch, Player p1, Player p2, InventoryManager invManager)
        {
            ...
            if (p1HoveredItem != null) AssetManager.DrawTextBottomCenter(spriteBatch, $"${p1HoveredItem.Value}", UIConfig.P1MarketValuePos, Color.Orange);

            Item p2HoveredItem = invManager.GetSelectedItem(p2, 2);
            if (p2HoveredItem != null) AssetManager.DrawTextBottomCenter(spriteBatch, $"${p2HoveredItem.Value}", UIConfig.P2MarketValuePos, Color.Orange);
            ...
            for (int y = 0; y < GameConstants.InventoryRows; y++)
            {
                for (int x = 0; x < GameConstants.InventoryCols; x++)
                {
                    ...
                    if (p.InventoryGrid[x, y] != null)
                        sb.Draw(AssetManager.Pixel, cellRect, Color.White * 0.05f);
                    else
